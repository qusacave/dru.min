...
onMuteButton = function() {
    ...
    if( nowMute ) {
        setMute( false );
	    nowMute = false;
        document.getElementById("imgButton").src = "./src/unmute.png";
    }
    ...
    //  ミュート状態だったらミュートにする
	    ...
	    setMute( true );
	    nowMute = true;
        document.getElementById("imgButton").src = "src/mute.png";
    }
};
...
//  ボタンを4つずつ配置する
keys = function(){
    var nKeys = Math.ceil( inst.length/4 );
    ...
    for(i=0; i<nKeys; i++){
        for(j=0; j<4; j++){
            ...
            ctx.fillText(iString[4*i+j], x, y+3);
        }
    }
